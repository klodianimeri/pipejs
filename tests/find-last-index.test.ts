...
import { findLastIndex } from '../src/pipes/find-last-index';

test('sync findLastIndex', () => {
    let pipe = pipeSync(toGeneratorSync([{ a: 6 }, { a: 5 }, { a: 6, b: 0 }, { a: 5 }]), findLastIndex(x => x.a === 6));
...
test('async findLastIndex', async () => {
    let pipe = pipeAsync(toGeneratorAsync([{ a: 6 }, { a: 5 }, { a: 6, b: 0 }, { a: 5 }]), findLastIndex(x => x.a === 6));
